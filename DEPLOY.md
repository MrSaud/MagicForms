...
**Source path:** the real checkout is `/Users/dev-saud/Desktop/SWAPProjects/MagicForms/` — not
`Desktop/DjangoLab/MagicForms` (an old, stale checkout from before the 2026 rewrite still exists
there; using it silently deploys outdated code with no error, since rsync just syncs whatever
directory you point it at). Double-check the source path before running this.

`config/settings.py`, `media/` (live user uploads — signatures, documents), `staticfiles/`
(regenerated by `collectstatic`), and native build artifacts are **excluded** so they are never
overwritten or wiped by a code sync.
...
rsync -azc \
  ...
  --exclude '.claude/' \
  /Users/dev-saud/Desktop/SWAPProjects/MagicForms/ \
  ec2-user@ec2-18-198-36-222.eu-central-1.compute.amazonaws.com:/var/www/magicforms/
```

Run with `-azcn` first (dry run) to review the file list before syncing for real.
